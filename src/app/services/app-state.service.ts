...
export class AppStateService {
  ...
  readonly hasDuplicates = computed(() => this.conversionState().duplicateNames.size > 0);

  /** Number of duplicate request names */
  readonly duplicateCount = computed(() => this.conversionState().duplicateNames.size);

  /** Whether conversion output exists */
  readonly hasOutput = computed(() => this.conversionState().output !== null);

  /** Number of parsed requests */
  readonly requestCount = computed(() => this.conversionState().requests.length);
